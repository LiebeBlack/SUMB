...
class MainViewModel:
    ...
    reemitir eventos de dominio. No consulta WMI ni manipula controles XAML.
    """
    ...
    def select_device(self, device: UsbDevice | None) -> None:
        if self._selected_device is device:
            return
    ...
    def _on_service_event(self, event: DeviceChangedEvent) -> None:
        if event.change_type is DeviceChangeType.device_added:
            ...
            if self._selected_device is not None and event.device is not None:
                if event.device.pnp_device_id == self._selected_device.pnp_device_id:
            ...
            logger.warning("Monitor error event: %s", event.message)
    ...
    def _sync_from_service(self) -> None:
        ...
        if self._selected_device is None and self._device_list:
            self._selected_device = self._device_list[0]
        if self._selected_device is not None and self._selected_device not in self._device_list:
            self._selected_device = None
        self._sync_selection_presence()

    def _sync_selection_presence(self) -> None:
        if self._selected_device is not None and self._selected_device in self._device_list:
            return
